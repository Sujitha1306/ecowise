import React, { useState, useEffect, useContext, useMemo, useCallback } from 'react';
import { AuthContext } from '../context/AuthContext';
// Import PieChart components
import { PieChart, Pie, Cell, Tooltip, Legend, ResponsiveContainer } from 'recharts';
import './DashboardStyles.css';

// Colors for our pie chart
const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#AF19FF', '#FF1943', '#FF4F81', '#4FFF81', '#814FFF'];

function DashboardPage() {
  const { user } = useContext(AuthContext);
  
  // State for dashboard stats
  const [stats, setStats] = useState({ current_streak: 0, total_savings: 0, consumption_breakdown: [] });
  
  // --- THIS SECTION IS NOW COMPLETE ---
  // State for appliance management
  const [myAppliances, setMyAppliances] = useState([]);
  const [allAppliances, setAllAppliances] = useState([]);
  // State for cascading dropdowns
  const [selectedType, setSelectedType] = useState('');
  const [selectedBrand, setSelectedBrand] = useState('');
  const [selectedApplianceId, setSelectedApplianceId] = useState('');
  
  // State for suggestions
  const [suggestion, setSuggestion] = useState(null);
  
  // Loading and error states
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);

  // Function to fetch all dashboard data, wrapped in useCallback for stability
  const fetchData = useCallback(() => {
    if (user) {
      setIsLoading(true);
      setError(null); // Clear previous errors on refetch
      Promise.all([
        fetch(`http://localhost:5000/user/${user.id}/stats`, { credentials: 'include' }),
        fetch(`http://localhost:5000/user/${user.id}`, { credentials: 'include' }),
        fetch('http://localhost:5000/appliances', { credentials: 'include' })
      ])
      .then(async ([statsRes, myAppliancesRes, allAppliancesRes]) => {
        if (!statsRes.ok || !myAppliancesRes.ok || !allAppliancesRes.ok) {
          throw new Error('Could not fetch all dashboard data.');
        }
        const statsData = await statsRes.json();
        const myAppliancesData = await myAppliancesRes.json();
        const allAppliancesData = await allAppliancesRes.json();
        
        setStats(statsData);
        setMyAppliances(myAppliancesData.appliances || []);
        setAllAppliances(allAppliancesData || []);
      })
      .catch(err => setError(err.message))
      .finally(() => setIsLoading(false));
    }
  }, [user]); // Dependency on user

  // Fetch data on initial load
  useEffect(() => {
    fetchData();
  }, [fetchData]);

  // Memoized lists for dropdowns
  const applianceTypes = useMemo(() => [...new Set(allAppliances.map(app => app.type))], [allAppliances]);
  const availableBrands = useMemo(() => selectedType ? [...new Set(allAppliances.filter(app => app.type === selectedType).map(app => app.brand))] : [], [allAppliances, selectedType]);
  const availableModels = useMemo(() => selectedBrand ? allAppliances.filter(app => app.type === selectedType && app.brand === selectedBrand) : [], [allAppliances, selectedType, selectedBrand]);

  const handleAddAppliance = (e) => {
    e.preventDefault();
    if (!selectedApplianceId) return;
    fetch(`http://localhost:5000/user/${user.id}/appliance`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ appliance_id: selectedApplianceId }),
    })
    .then(res => { if (!res.ok) throw new Error("Failed to add appliance."); return res.json(); })
    .then(() => fetchData())
    .catch(err => setError(err.message));
  };

  const handleRemoveAppliance = (applianceId) => {
    fetch(`http://localhost:5000/user/${user.id}/appliance/${applianceId}`, {
      method: 'DELETE',
      credentials: 'include',
    })
    .then(res => { if (!res.ok) throw new Error('Failed to remove appliance'); return res.json(); })
    .then(() => fetchData())
    .catch(error => setError(error.message));
  };
  
  const getSuggestion = (applianceId) => {
    setSuggestion(null);
    fetch(`http://localhost:5000/user/${user.id}/appliance/${applianceId}/suggestion`, { credentials: 'include' })
      .then(res => res.json())
      .then(data => { if (data.error) throw new Error(data.error); setSuggestion(data); })
      .catch(err => setError(err.message));
  };
  
  const handleAcceptSuggestion = () => {
    if (!suggestion || !suggestion.savings) return;
    fetch('http://localhost:5000/suggestion/accept', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ savings: suggestion.savings })
    })
    .then(res => res.json())
    .then(data => {
        setSuggestion(null);
        fetchData();
        alert(`Great job! Your Eco-Streak is now ${data.new_streak}!`);
    })
    .catch(err => setError("Could not log your action. Please try again."));
  }

  if (isLoading) return <div className="dashboard-container"><p>Loading dashboard...</p></div>;
  if (!user) return <div className="dashboard-container"><p className="error-message">Please log in to continue.</p></div>;

  return (
    <div className="dashboard-container">
      <h2>Welcome, {stats.username}!</h2>
      
      {error && <p className="error-message">{error}</p>}
      
      <div className="stats-grid">
        <div className="stat-card">
          <h3>🔥 Eco-Streak</h3>
          <p className="stat-value">{stats.current_streak} Day{stats.current_streak !== 1 && 's'}</p>
        </div>
        <div className="stat-card">
          <h3>💰 Total Savings</h3>
          <p className="stat-value">₹{stats.total_savings.toFixed(2)}</p>
        </div>
      </div>

      <div className="chart-container">
        <h3>Energy Consumption Breakdown</h3>
        {stats.consumption_breakdown && stats.consumption_breakdown.length > 0 ? (
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={stats.consumption_breakdown} dataKey="consumption_kwh" nameKey="type" cx="50%" cy="50%" outerRadius={100} label>
                {stats.consumption_breakdown.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                ))}
              </Pie>
              <Tooltip formatter={(value) => `${value.toFixed(2)} kWh`} />
              <Legend />
            </PieChart>
          </ResponsiveContainer>
        ) : (
          <p>Add an appliance to see your energy breakdown.</p>
        )}
      </div>

      {/* --- THIS SECTION IS NOW COMPLETE --- */}
      <div className="management-section">
        <h3>Manage My Appliances</h3>
        <div className="add-appliance-section">
          <h4>Add a New Appliance</h4>
          <form onSubmit={handleAddAppliance} className="add-appliance-form">
              <select value={selectedType} onChange={e => { setSelectedType(e.target.value); setSelectedBrand(''); setSelectedApplianceId(''); }}>
                  <option value="">1. Select Type</option>
                  {applianceTypes.map(type => <option key={type} value={type}>{type}</option>)}
              </select>
              <select value={selectedBrand} onChange={e => { setSelectedBrand(e.target.value); setSelectedApplianceId(''); }} disabled={!selectedType}>
                  <option value="">2. Select Brand</option>
                  {availableBrands.map(brand => <option key={brand} value={brand}>{brand}</option>)}
              </select>
              <select value={selectedApplianceId} onChange={e => setSelectedApplianceId(e.target.value)} disabled={!selectedBrand}>
                  <option value="">3. Select Model</option>
                  {availableModels.map(app => <option key={app.id} value={app.id}>{app.model}</option>)}
              </select>
              
              <button type="submit" disabled={!selectedApplianceId}>Add</button>
          </form>
        </div>
        <div className="appliances-list">
          <h4>My Appliances</h4>
          {myAppliances.length > 0 ? (
            myAppliances.map(appliance => (
              <div key={appliance.id} className="appliance-item">
                <span>{appliance.brand} {appliance.model}</span>
                <div className="button-group">
                  <button className="advice-button" onClick={() => getSuggestion(appliance.id)}>Get Advice</button>
                  <button className="remove-button" onClick={() => handleRemoveAppliance(appliance.id)}>Remove</button>
                </div>
              </div>
            ))
          ) : (
            <p>You haven't added any appliances yet.</p>
          )}
        </div>
      </div>

      {suggestion && (
        <div className="suggestion-card-dashboard">
          <h3>Smart Suggestion For:</h3>
          <p><strong>{suggestion.appliance}</strong></p>
          <p className="suggestion-text">{suggestion.suggestion}</p>
          <button className="accept-button" onClick={handleAcceptSuggestion}>
            I did this! 
          </button>
        </div>
      )}
    </div>
  );
}

export default DashboardPage;

